...
const ArGetStarted: React.FC<IProps> = ({ data }) => {
    ...
    <div dir="rtl">
      <p className="relative mb-5 mt-[70px] text-center font-heading text-h1-mobile sm:mt-20 sm:text-h1-tablet md:mt-[95px] lg:mt-[75px] lg:text-h1 text-ink-900">
        <Image
          ...
          alt=""
          aria-hidden="true"
          className="absolute -top-5 right-[10%] z-10 h-[50px] w-[50px] object-contain sm:hidden"
        />
        <Image
          src={linesInvert}
          alt=""
          aria-hidden="true"
          className="absolute z-10 hidden h-[35px] w-[43px] object-contain sm:-top-10 sm:right-[10%] sm:block md:right-[23%] lg:-top-[30px] lg:right-[33%]"
        />
        ابدأ في <span className="text-brand-500">3</span> خطوات سهلة!
      </p>

      <div className="mx-auto hidden w-[90%] flex-row items-center justify-center lg:flex">
        <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data?.map((item, index) => (
            <ArGetStartedBox key={index} {...(item as any)} />
          ))}
        </div>
      </div>
...
const ArGetStartedBox: React.FC<Props> = ({ heading, description, image, ButtonText }) => (
