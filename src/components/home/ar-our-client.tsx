...
const ArOurClient: React.FC<IProps> = ({ data }) => {
  ...
  return (
    <Box sx={styles.constanier} dir="rtl">
      <Box sx={styles.inner}>
        <Box sx={styles.headingContanier}>
          ...
          <Typography
            ...
            className={leagueSpartan.className}
          >
            استمع إلى التجارب المذهلة التي يشاركها طلابنا!
            سجل الآن لفتح عالم غني من التعلم الممتع
          </Typography>
          <Button
            variant="contained"
      ...
      <Box sx={styles.ourStudent}>
        ...
        >
          {data.map((item, index) => (
            <SwiperSlide
              key={index}
              style={{ display: "flex", borderRadius: 10, paddingRight: "12vw" }}
            >
              <ArReview item={item} />
            </SwiperSlide>
          ))}
...
const ArReview: React.FC<RProps> = ({ item }) => {
  return (
    <Box
      sx={{
        display: "flex",
...
const ArReviewMobile: React.FC<RProps> = ({ item }) => {
  return (
    <Box sx={styles.reviewContanier}>
      <Box sx={styles.ratingContanier}>
        <StarPurple500OutlinedIcon
