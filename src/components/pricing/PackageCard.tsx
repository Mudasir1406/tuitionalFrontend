...
const PackageCard: React.FC<PackageCardProps> = memo(
  ({ package: pkg, userCountry, locale = "en", isPopular = false, sessionType = "online" }) => {
    const [showAllFeatures, setShowAllFeatures] = useState(false);
    const userPrice = pkg.pricing[userCountry];

    const { displayPrice, currencySymbol, basePrice, discount } = useMemo(() => {
      if (!userPrice) {
        return { displayPrice: 0, currencySymbol: "", basePrice: 0, discount: 0 };
      }
    ...
    return (
      ...
      >
        ...
        <div className="mb-6 border-b border-ink-200 pb-4">
          ...
          <p className="font-heading text-small text-ink-700">per session</p>
          {sessionType === "online" && basePrice !== displayPrice && discount > 0 && (
            <p className="mt-1 font-heading text-small">
              ...
              <span className="text-success font-semibold">Save {discount}%</span>
            </p>
          )}
        </div>
        ...
        <div className="flex-1">
          <div className="flex flex-col gap-2">
            ...
            {pkg.features && pkg.features.length > 5 && (
              ...
              >
                {showAllFeatures ? "Show less" : `+${pkg.features.length - 5} more features`}
              </button>
            )}
          </div>
        </div>
