...
| Check | Expected |
|---|---|
| 1.1 | Only approved libraries (`tailwindcss`, `@headlessui/react`, `lucide-react`, `clsx`, `tailwind-merge`, `swiper`, `moment`, `axios`, `firebase`, `react-hot-toast`, `cheerio`, `react-phone-number-input`, `wavesurfer.js`, `country-flag-icons`) |
| 1.2 | No `@mui/*`, `@emotion/*`, `stylis-plugin-rtl` reintroduced — they were removed in 2026 |
| 1.3 | No Bootstrap / SCSS / additional design system / animation library / state manager added |
| 1.4 | No rich-text editor, no drag-and-drop library — this repo does not use them |
| 1.5 | No alternative icon pack (`react-icons`, `phosphor-icons`, Heroicons) — `lucide-react` only |
| 1.6 | Package manager commands use `npm`, not `yarn` |
...
| Check | Expected |
|---|---|
| 2.1 | The component is styled with Tailwind utility classes — no `sx`, no `<Box>`, no `<Typography>` |
| 2.2 | Conditional class merging uses `cn()` from `@/utils/cn` — no string concatenation of classNames |
| 2.3 | No new `*.module.css` files (existing ones are grandfathered — TextEditor, TrustpilotCarousel, route-shell `.module.css`) |
| 2.4 | No edits to [globals.css](src/app/globals.css) |
| 2.5 | No `@mui/*` / `@emotion/*` import; no `styled()` / `makeStyles`; no `theme.palette.*` / `theme.spacing(n)` |
| 2.6 | No `em` units anywhere; no `vw` for font-size |
| 2.7 | No inline `style={{}}` except for runtime-computed values, `next/image` sizing, dynamic background images, or preserved legacy boxShadow strings |
| 2.8 | Direction-sensitive spacing/positioning uses logical properties (`ms-*`, `me-*`, `ps-*`, `pe-*`, `start-*`, `end-*`, `text-start`, `text-end`) — not `ml-*` / `mr-*` / `flex-row-reverse` driven by `isRTL` |

### ✅ DO
- Grep the new file for `@mui`, `@emotion`, `styled(`, `makeStyles`, `theme.palette`, `theme.spacing`, `em`, `vw`, ` sx={`, ` flex-row-reverse` — all should return zero.

### ❌ DON'T
- Don't reintroduce `@mui/*` / `@emotion/*` imports — those packages are uninstalled.

**If this fails →** [UI.md §4](./UI.md) has the clean Tailwind template.
...
| Check | Expected |
|---|---|
| 3.1 | Brand blue: `bg-brand-500` / `text-brand-500` (`#38b6ff`) |
| 3.2 | Section wash: `bg-brand-50` (`#D7F0FF`) |
| 3.3 | Body text: `text-ink-900` (`#2d2d2d`); secondary: `text-ink-700` |
| 3.4 | Neutrals outside pricing: ink ramp tokens — no improvised hex |
| 3.5 | Pricing slate used ONLY inside [components/pricing/](src/components/pricing/) |
| 3.6 | No new arbitrary `bg-[#xxx]` / `text-[#xxx]` for tokens that already exist |

### Typography

| Check | Expected |
|---|---|
| 3.7 | Headings use `text-h1` / `text-h2` / etc. paired with mobile/tablet variants (`text-h1-mobile sm:text-h1-tablet lg:text-h1`) |
| 3.8 | Body text uses `text-body` or `text-small` — no improvised rem values |
| 3.9 | Font family set via `font-heading` / `font-body` / `font-arabic` Tailwind classes |
| 3.10 | Input `font-size` never drops below `16px` on mobile (iOS zoom prevention) |

### Shadows & Radii

| Check | Expected |
|---|---|
| 3.11 | Shadows match the table in [Design.md §3](./Design.md) — `shadow-card`, `shadow-card-hover`, or arbitrary values copied verbatim |
| 3.12 | Radii in `{rounded, rounded-lg, rounded-[10px], rounded-xl, rounded-2xl, rounded-[20px], rounded-full}` — no invented values |
...
| 5.2 | Direction-sensitive spacing/positioning uses logical Tailwind utilities (`ms-*`, `me-*`, `ps-*`, `pe-*`, `start-*`, `end-*`, `text-start`, `text-end`) |
| 5.3 | No `isRTL ? "ml-4" : "mr-4"` / `flex-row-reverse` ternaries for simple direction flips — let logical properties handle it |
| 5.4 | OR — a paired `ar-*.tsx` twin file handles AR layout when LTR and AR genuinely diverge |
| 5.5 | When editing a shared concept, the Arabic twin (if any) was updated in lockstep |
| 5.6 | New `/route` has an `/ar/route` mirror (or an explicit documented reason it's EN-only) |
| 5.7 | AR text renders in Noto Sans Arabic (handled by `html[dir="rtl"] *` in [globals.css](src/app/globals.css) and the `font-arabic` Tailwind class) |
...
| 7.7 | Added redirects live in [next.config.mjs](next.config.mjs) `redirects()`, not via `redirect()` in a route file |
...
| 9.7 | Breakpoints match Tailwind config: `sm:600 / md:900 / lg:1200 / xl:1500 / 2xl:2000` |
| 9.8 | Mobile-first authoring: default styles target mobile, `sm:`, `md:`, `lg:`, `xl:`, `2xl:` layer up |
...
| Check | Expected |
|---|---|
| 10.1 | No second `I18nProvider` / `DrawerProvider` mounted (all at root in [layout.tsx](src/app/layout.tsx)) |
| 10.2 | Only one `<Toaster />` mounted |
| 10.3 | Context consumption via hooks (`useI18n()`, `useDrawer()`) — no prop-drilling |
| 10.4 | No reintroduction of `ThemeProvider` from `@mui/material` — MUI is uninstalled |

### ✅ DO
- Grep for `I18nProvider`, `DrawerProvider`, `<Toaster` — each should appear once ([layout.tsx](src/app/layout.tsx)).

### ❌ DON'T
- Don't wrap a subtree in a second provider.
- Don't reintroduce a `ThemeProvider` — Tailwind config drives all design tokens.
...
| Check | Expected |
|---|---|
| 11.1 | Loading states use Tailwind animation utilities (e.g. `animate-pulse` skeletons) or a small purpose-built spinner — no MUI `CircularProgress` |
...
| 11.5 | Hover / focus states visible and accessible (Tailwind `hover:`, `focus-visible:`) |
| 11.6 | Keyboard navigation: Tab order sensible, Enter/Space triggers interactive elements (Headless UI primitives handle this for Dialog/Listbox/Disclosure) |
...
| Check | Expected |
|---|---|
| 12.1 | Legacy misspelled files unchanged: `form-dialouge.tsx`, `form-dialouge-v1.tsx`, `students-says.tsx`, etc. |
